import openmc
import openmc.model.RectangularParallelepiped as RPP
...
def baby_model(side='both'):
    ...
    if source_side == 'west':
        ...
        libra_center_coord = [(experiment_rpp.xmin.x0 + center_shield_rpp.xmin.x0)/2,
                            ...
                            100]
        _, cllif_cell_west, cells_west = baby_geometry(*libra_center_coord) 
        cllif_cells = [cllif_cell_west]
        src = A325_generator_diamond((libra_center_coord[0], libra_center_coord[1], libra_center_coord[2] - 5.635), (1, 0, 0))
        ...
        libra_center_coord = [(center_shield_rpp.xmax.x0 + experiment_rpp.xmax.x0)/2,
                            experiment_rpp.ymin.y0 + 100,
                            100]
        _, cllif_cell_east, cells_east = baby_geometry(*libra_center_coord)
        cllif_cells = [cllif_cell_east]
        src = A325_generator_diamond((libra_center_coord[0], libra_center_coord[1], libra_center_coord[2] - 5.635), (1, 0, 0))
    elif source_side == 'both':
        libra_center_coord_west = [(experiment_rpp.xmin.x0 + center_shield_rpp.xmin.x0)/2,
                            experiment_rpp.ymin.y0 + 100,
                            100]
        libra_center_coord_east = [(center_shield_rpp.xmax.x0 + experiment_rpp.xmax.x0)/2,
                            experiment_rpp.ymin.y0 + 100,
                            100]
        _, cllif_cell_west, cells_west = baby_geometry(*libra_center_coord_west) 
        _, cllif_cell_east, cells_east = baby_geometry(*libra_center_coord_east)
        cllif_cells = [cllif_cell_west, cllif_cell_east]
        src_west = A325_generator_diamond((libra_center_coord_west[0], libra_center_coord_west[1], libra_center_coord_west[2] - 5.635), (1, 0, 0))
        src_east = A325_generator_diamond((libra_center_coord_east[0], libra_center_coord_east[1], libra_center_coord_east[2] - 5.635), (1, 0, 0))

        src = [src_west, src_east]
    ...
    src = A325_generator_diamond((libra_center_coord[], y_c, z_c - 5.635), (1, 0, 0))
    settings.source = src
    ...
    model = build_bates_model(
        experiment_universe=openmc.Universe(cells=cells),
        source_room="cask",
